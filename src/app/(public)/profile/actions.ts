...
export async function addPoints(
  ...
  const { data: profile, error: profileError } = await supabase
    .from('profiles')
    .select('total_points, current_points, lifetime_points')
    .eq('id', userId)
    .single()
  ...
  const newLifetimePoints = profile.lifetime_points + points

  // Update profile với giá trị mới đã tính toán
...
export async function getRankHistory() {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      from_rank:ranks!rank_history_from_rank_id_fkey(*),
      to_rank:ranks!rank_history_to_rank_id_fkey(*)
    `)
    .eq('user_id', user.id)
